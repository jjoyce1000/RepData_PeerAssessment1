...
```
*Mean =    `r as.integer(mean(sum_steps_by_day_df$steps))`*  
*Median =  `r as.integer(median(sum_steps_by_day_df$steps))`*  
...
```
*Maximum number of steps in a 5-min interval =    `r round(max_steps,digits=2)`*  
*5-min interval with maximum number of steps =    `r max_interval`* 
```{r echo=TRUE, fig.align='center'}
...
```
*Total number of missing values (NAs) =    `r rows_NA`* 
...
```
*Mean =    `r as.integer(mean(sum_steps_by_day_imputed_df$imputed.value))`*  
*Median =  `r as.integer(median(sum_steps_by_day_imputed_df$imputed.value))`* 
...
##  averaged across all weekday days or weekend days (y-axis) using the imputed data (combined_df).
    ...
    plot(mean_steps_by_interval_weekday_df$interval,mean_steps_by_interval_weekend_df$imputed.value,type="l",
         ...
         ylab = "Average # of Steps",col="blue")
```  
*Maximum number of steps in a 5-min interval (Weekday) =    `r round(max(mean_steps_by_interval_weekday_df$imputed.value),digits=2)`*  
*Maximum number of steps in a 5-min interval (Weekend) =    `r round(max(mean_steps_by_interval_weekend_df$imputed.value),digits=2)`*  